...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const loginUser = useCallback((data: AuthResponse) => {
    // some backends may return role as string; normalize to number to avoid
    // strict-equality bugs (e.g. comparing against ADMIN_ROLE)
    const roleNum = Number(data.role);
    persistAuth({ ...data, role: roleNum });
    setUser({
      token: data.token,
      role: roleNum,
      username: data.username,
